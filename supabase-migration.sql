...
-- Community gas price reports. Reports stay pending until reviewed by an admin.
create table if not exists public.station_price_reports (
  id bigint generated by default as identity primary key,
  station_id text not null,
  station_name text not null,
  lat double precision not null,
  lng double precision not null,
  fuel_type text not null check (fuel_type in ('87', '90', 'diesel')),
...
alter table public.station_price_reports enable row level security;

drop policy if exists "Anyone can submit gas price reports" on public.station_price_reports;
create policy "Anyone can submit gas price reports"
  on public.station_price_reports for insert
  to anon, authenticated
  with check (status = 'pending');

drop policy if exists "Anyone can view approved gas price reports" on public.station_price_reports;
create policy "Anyone can view approved gas price reports"
  on public.station_price_reports for select
  to anon, authenticated
