  ...
  (function() {
    ...
    fetch(BASE + '/ads?id=eq.' + adId + '&select=views', { headers: H })
      ...
      .then(function(d) {
        ...
        show(seen ? cur : cur + 1); // optimistic +1

        if (seen) return; // already counted this session
        ...
        fetch(BASE + '/rpc/increment_view', {
          ...
          if (r.ok) {
            ...
            // RPC not available — fall back to direct PATCH
            // (works if anon_update_views RLS policy is in place)
            fetch(BASE + '/ads?id=eq.' + adId, {
              method: 'PATCH',
              headers: Object.assign({}, H, { 'Prefer': 'return=minimal' }),
              body: JSON.stringify({ views: cur + 1 })
            }).then(function(r2) {
              if (r2.ok) sessionStorage.setItem(sKey, '1');
              else show(cur); // both failed — revert
            }).catch(function() { show(cur); });
          }
        }).catch(function() { show(cur); });
      }).catch(function() {});
  })();
