...
<script>
  ...
  // ── View counter — RPC increment + display ───────────────────
  // Calls a Postgres function (security definer) that bypasses RLS.
  // sessionStorage dedupes refreshes — counts once per browser session.
  (function() {
    var el   = document.getElementById('viewCountEl');
    var adId = '${ad.id}';
    var SBASE = 'https://cquwshpsfybvgqodbxsf.supabase.co';
    ...
    var sessKey = 'ya_viewed_' + adId;

    // Always fetch current count to display
    fetch(SBASE + '/rest/v1/ads?id=eq.' + adId + '&select=views', { headers: HDRS })
      .then(function(r) { return r.json(); })
      .then(function(data) {
        var current = (data && data[0] && data[0].views != null) ? Number(data[0].views) : 0;
        // Show current + 1 optimistically if we are about to increment
        var willIncrement = !sessionStorage.getItem(sessKey);
        if (el) el.textContent = (willIncrement ? current + 1 : current).toLocaleString('en-JM');

        if (willIncrement) {
          // Call the security-definer RPC — bypasses RLS, atomic increment
          fetch(SBASE + '/rest/v1/rpc/increment_view', {
            method: 'POST',
            headers: HDRS,
            ...
            if (r.ok) {
              ...
              // RPC failed — revert optimistic display
              if (el) el.textContent = current.toLocaleString('en-JM');
              r.text().then(function(t) { console.warn('view rpc:', t); });
            }
          }).catch(function() {
            if (el) el.textContent = current.toLocaleString('en-JM');
          });
