...
<style>
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .float-btn-call {
    background: linear-gradient(155deg, var(--green-mid), var(--green));
    color: #fff;
  ...
  .status-badge.active { background: rgba(0,92,53,0.15); color: var(--green); }
  ...
  .seller-avatar {
    ...
    background: linear-gradient(135deg, var(--green), var(--green-mid)); color: #fff;
  }
...
<script>
  ...
  window.addEventListener('load', function() {
    ...
    // matchMedia is Chromium-only — guard so iOS Safari/Firefox can't crash
    // this script (which would kill ad-page interactivity).
    try {
  ...
  (function() {
    ...
    /* §DEBUG-GATE — console output ships only when the visitor opts in
       (?debug=1 or localStorage ya_debug=1). Production stays silent so
       ordinary visits never leak internals to DevTools. */
    function yaDebugOn() {
      try {
        if (localStorage.getItem('ya_debug') === '1') return true;
        if (typeof location !== 'undefined' && /(?:\?|&)debug=1(?:&|$)/.test(location.search || '')) return true;
      } catch (e) {}
      return false;
    }
    function dbg(msg) { if (yaDebugOn()) { try { console.log('[YaadAdz views]', msg); } catch (e) {} } }
