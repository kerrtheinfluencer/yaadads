...
<script>
  ...
  // ── View counter — increment on page load, update display ──────
  // Uses sessionStorage to avoid double-counting refreshes.
  (function() {
    var el    = document.getElementById('viewCountEl');
    var adId  = '${ad.id}';
    var SBASE = 'https://cquwshpsfybvgqodbxsf.supabase.co/rest/v1';
    ...
    // Dedupe: only count once per session per ad
    var sessKey = 'viewed_' + adId;
    var alreadyCounted = sessionStorage.getItem(sessKey);

    // Step 1: fetch current view count to display immediately
    fetch(SBASE + '/ads?id=eq.' + adId + '&select=views', { headers: HDRS })
      .then(function(r) { return r.json(); })
      .then(function(data) {
        var current = (data && data[0] && data[0].views != null) ? Number(data[0].views) : null;
        if (current === null) return;

        if (!alreadyCounted) {
          // Step 2: increment by 1 using PATCH with the new value
          var newViews = current + 1;
          fetch(SBASE + '/ads?id=eq.' + adId, {
            method: 'PATCH',
            ...
            if (r.ok) {
              ...
              if (el) el.textContent = newViews.toLocaleString('en-JM');
            } else {
          ...
          // Already counted this session — just display current
          if (el) el.textContent = current.toLocaleString('en-JM');
        }
      }).catch(function() {});
  })();
